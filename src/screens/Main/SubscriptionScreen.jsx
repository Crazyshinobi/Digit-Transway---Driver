...
//     setIsLoading(true);
//     try {
//       const { data } = await axios.get(`${TEST_API_URL}/api/vendor-plans/`);
//       if (data?.success) {
...
const SubscriptionScreen = ({ navigation, route }) => {
  ...
  const verifyPaymentOnServer = async (paymentData, subscriptionId) => {
    ...
    console.log('═══════════════════════════════════════');
    console.log('📍 Endpoint:', `${TEST_API_URL}/api/vendor-plans/verify-payment`);
    console.log('🕒 Timestamp:', new Date().toISOString());
    console.log('📦 Payment Data Received:');
    console.log('   - razorpay_payment_id:', paymentData.razorpay_payment_id);
    console.log('   - razorpay_subscription_id:', paymentData.razorpay_subscription_id);
  ...
  const subscribeToPlan = async planId => {
    ...
    try {
      ...
      console.log('🔑 Authorization:', accessToken ? `Bearer ${accessToken.substring(0, 20)}...` : 'NULL');
      ...
      if (response.data.success) {
        ...
        console.log('─────────────────────────────────────');
        console.log('🔑 Razorpay Key:', razorpay_key ? `${razorpay_key.substring(0, 15)}...` : 'NULL');
        console.log('🆔 Razorpay Subscription ID:', razorpay_subscription_id);
        console.log('💰 Amount (paise):', amount_in_paise);
        console.log('📋 Plan Name:', plan_name);
        console.log('🆔 Local Subscription ID:', subscription_id);
        ...
        const options = {
          description: plan_name || 'Vendor Subscription',
          currency: 'INR',
          key: razorpay_key,
